...
function toggleTheme() {
    ...
    if (btnTema) {
        btnTema.innerHTML = isDark ? '<i class="fas fa-sun"></i> Mode Terang' : '<i class="fas fa-moon"></i> Mode Gelap';
    }
}
...
// Modal Info
if (btnInfoBuka) btnInfoBuka.addEventListener('click', () => { modalInfo.style.display = 'flex'; });
if (btnCloseModal) btnCloseModal.addEventListener('click', () => { modalInfo.style.display = 'none'; });
window.addEventListener('click', (e) => { if (e.target == modalInfo) modalInfo.style.display = 'none'; });

/* =========================================
   3. LOGIKA KALKULATOR (HITUNG & RESET)
   ========================================= */
// ... (Kode hitungBMI dan resetCalculator Anda yang sudah ada tetap di sini) ...
...
function hitungBMI() {
    ...
    if (bmi < 18.5) {
        kategori = 'kurang berat badan'; warna = '#e74c3c';
        saran = 'Anda berada di kategori kurang berat badan. Tingkatkan asupan kalori dengan makanan bergizi seimbang.';
    } else if (bmi >= 18.5 && bmi <= 24.9) {
        kategori = "berat badan normal"; warna = '#04ff00';
        saran = 'Luar biasa! Berat badan Anda ideal. Pertahankan pola makan sehat.';
    } else if (bmi >= 25 && bmi <= 29.9) {
        kategori = 'kelebihan berat badan'; warna = '#ff8400';
        saran = 'Anda masuk kategori kelebihan berat badan. Cobalah untuk lebih aktif bergerak.';
    } else {
        kategori = 'obesitas'; warna = '#7a0606';
...
function resetCalcuator() {
    inputBerat.value = ''; inputTinggi.value = '';
    ...
    boxIdeal.style.display = 'none'; boxSaran.style.display = 'none';
}